...
export default function CartPage() {
  ...
  const checkoutAttempt = useRef<{ delivery: DeliveryEmailInput; idempotencyKey: string } | null>(null);

  if (authLoading) return <main className="min-h-screen bg-slate-50 px-4 pb-20 pt-20"><ApiLoading label="Restoring your cart…" /></main>;
  if (!user || user.role !== "CUSTOMER") return <main className="min-h-screen bg-slate-50 px-4 pb-20 pt-20"><div className="mx-auto max-w-4xl rounded-[2rem] border border-slate-200 bg-white p-12 text-center"><ShoppingBag className="mx-auto h-12 w-12 text-blue-600" /><h1 className="mt-5 text-3xl font-black text-slate-950">Sign in to view your cart</h1><p className="mt-3 text-sm text-slate-500">Your authenticated cart is stored securely on the server.</p><Link href="/login?returnTo=%2Fcart" className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-bold text-white">Sign in<ArrowRight className="h-4 w-4" /></Link></div></main>;

  const items = query.data?.items ?? [];
  const submitCheckout = (delivery: DeliveryEmailInput) => {
    if (checkout.isPending) return;
    if (!checkoutAttempt.current) {
      checkoutAttempt.current = { delivery, idempotencyKey: createCheckoutIdempotencyKey() };
    }
    checkout.mutate(checkoutAttempt.current, {
    onSuccess: order => {
      setDeliveryOpen(false);
      rememberPaymentAttempt({ orderId: order.orderId, paymentId: order.paymentId, paymentExpiresAt: order.paymentExpiresAt, userId: user.id });
      window.location.assign(order.paymentUrl);
    },
    onError: error => {
      if (!(error instanceof ApiError && error.status === 0)) checkoutAttempt.current = null;
      if (["GIFT_CARD_OUT_OF_STOCK", "INSUFFICIENT_GIFT_CARD_STOCK", "INVALID_CART"].includes(getGiftCardErrorCode(error) || "")) query.refetch();
      toast.error("Could not start payment", getGiftCardErrorMessage(error));
    },
    });
  };
  ...
  const openCheckout = () => {
    if (user.isEmailVerified === false) {
      ...
      router.push("/verify-email?returnTo=%2Fcart");
      return;
    }
    setDeliveryOpen(true);
  };

  return <main className="min-h-screen bg-slate-50 pb-20 pt-20"><section className="bg-slate-950 text-white"><div className="mx-auto w-11/12 max-w-7xl py-12"><span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-[.18em]"><ShoppingBag className="h-3.5 w-3.5" />Secure cart</span><h1 className="mt-4 text-4xl font-black tracking-tight">Review your digital order.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Availability and prices are revalidated by the backend at checkout. The returned order total is authoritative.</p></div></section><div className="mx-auto w-11/12 max-w-7xl py-10">
