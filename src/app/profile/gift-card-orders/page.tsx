...
function Badge({ value }: { value: string }) {
  ...
  return <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${tone}`}>{value.replaceAll("_", " ")}</span>;
}
...
export default function GiftCardOrdersPage() {
  ...
  return <RouteGuard roles={["CUSTOMER"]}><main className="min-h-screen bg-slate-50 pb-20 pt-[132px] sm:pt-[148px]"><div className="mx-auto w-11/12 max-w-6xl py-10"><div className="mb-8 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.18em] text-blue-700">Your account</p><h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Gift-card orders</h1><p className="mt-3 text-sm text-slate-500">Codes stay hidden here. Open an owned completed order to reveal delivered codes.</p></div><Link href="/gift-cards" className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-bold text-white">Browse gift cards<ArrowRight className="h-4 w-4" /></Link></div>
