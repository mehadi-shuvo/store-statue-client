...
const mocks = vi.hoisted(() => ({ add: vi.fn(), push: vi.fn(), success: vi.fn(), error: vi.fn(), warning: vi.fn(), refetch: vi.fn() }));
vi.mock("next/image", () => ({ default: (props: { alt: string }) => <span role="img" aria-label={props.alt} /> }));
vi.mock("next/navigation", () => ({ useRouter: () => ({ push: mocks.push }) }));
vi.mock("@/context/AuthContext", () => ({ useAuth: () => ({ user: { id: "u1", email: "owner@example.com", role: "CUSTOMER" } }) }));
...
vi.mock("@/hooks/api/use-gift-card-api", () => ({
  ...
  useInstantGiftCardBuy: () => ({ isPending: false, mutate: vi.fn() }),
}));

describe("GiftCardProductDetails", () => {
  beforeEach(() => { mocks.add.mockReset().mockResolvedValue({}); mocks.success.mockReset(); });
